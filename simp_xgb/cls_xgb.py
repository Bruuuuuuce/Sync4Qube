...
if __name__ == '__main__':
    ...
    def get_Xdata_Y(factor_dict, return_data, date_list, if_train, multi, date=None):
        ...
        if if_train:
            kickout_small_num = len(np.where((YData - YData.mean()).abs() < cfg.clean_std_small_range * YData.std())[0])
            kickout_large_num = len(np.where((YData - YData.mean()).abs() > cfg.clean_std_large_range * YData.std())[0])
            YData.iloc[:] = np.where(1 - ((YData - YData.mean()).abs() >= cfg.clean_std_small_range * YData.std()) * ((YData - YData.mean()).abs() <= cfg.clean_std_large_range * YData.std()), np.nan, YData)

            print('kick out small ratio: {}%, kick out big ratio: {}%'.format(round(100 * kickout_small_num / YData.shape[0], 2), round(100 * kickout_large_num / YData.shape[0], 2)))

        # 清nan比例较高的样本
        na_pct = np.sum(np.isnan(XData), axis=1) / XData.shape[1]
        cut_na_points = (na_pct > cfg.cls_na_pct).values.reshape(YData.shape)
        ...
        YData.iloc[:] = np.where(YData > 0, 1, 0)
    ...
    def main_loop(train_start, valid_periods, test_periods, loss, start_date, end_date, factor_list, path, return_name,
        ...
        while test_start < len(model_date_list):
            t_start = time.time()
            if 'expand' in cfg.cls_mode:
                train_dates = model_date_list[:valid_start]
            elif 'roll' in cfg.cls_mode:
        ...
        diy_name = f'eod_xgb_{curr_date + curr_time}_cls'
        ...
        return predict_return_data


    memo = cfg.cls_memo
    print(memo)
    predict_return_data = main_loop(train_start=240,
                                    valid_periods=10,
                                    test_periods=40,
                                    loss='xgbreg_loss_function_mse',
                                    start_date='20160101',
                                    end_date='20210226',
                                    factor_list=cfg.cls_factor_list,
                                    path=cfg.cls_factor_path,
