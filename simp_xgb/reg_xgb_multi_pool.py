...
if __name__ == '__main__':
    ...
    # 读取收益率
    def get_return_data(return_name, return_path, mask_df=None):
        return_data = ds.get_eod_feature(fields=[return_name],
                                         where=return_path,
                                         tickers=tickers,
                                         dates=date_list)[return_name].to_dataframe() * mask_df
    ...
    def main_loop(train_start, valid_periods, test_periods, loss, start_date, end_date, factor_list, path, return_name,
        ...
        return_data, mask = get_return_data(return_name=return_name, return_path=return_path, mask_df=mask_df)
    ...
    result_dict = {}
    for type in ['TA', 'TB', 'TC', 'TD', 'TA+TB', 'TA+TC', 'TB+TC', 'TA+TB+TC']:
        type_list = type.split('+')
        if len(type_list) == 3:
            mask_df = (mask_dict['TA'] + mask_dict['TB'] + mask_dict['TC']).replace(0, np.nan)
        if len(type_list) == 2:
            mask_df = (mask_dict[type_list[0]] + mask_dict[type_list[1]]).replace(0, np.nan)
        if len(type_list) == 1:
            mask_df = mask_dict[type_list[0]].replace(0, np.nan)
        else:
            print('Wrong!')
            break
